...
if ($action == "ordercat") {
    ...
    $move = (isset($_POST['move'])) ? trim($_POST['move']) : trim($_GET['move']);
...
if ($action == "modifycats") {
    ...
    if (isset($_GET['opencat'])) {
        $opencat = intval($_GET['opencat']);
        $GLOBALS['map'][$opencat] = $opencat;
    }

    if (isset($_GET['closecat'])) {
        $closecat = intval($_GET['closecat']);
